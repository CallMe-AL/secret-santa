...
const SantaGenerator = () => {
  ...
  const removePerson = (e) => {
    if (participants.length > 3) {
      ...
      setWarning(true); 
      setWarningText("You need at least 3 people for a Secret Santa event!");       
    }
  }
  ...
  const createList = () => {
    ...
    participants.forEach((person, index) => {
      ...
      if (person.name === '') {
        setWarning(true);
        setWarningText(`Person ${index + 1} is nameless :(. Please edit or remove person.`);
        person.warning = true;
        nameCheck = false;
      } else if (currentNames.indexOf(person.name) !== -1) {
        setWarning(true);
        setWarningText(`Person ${index + 1} is a duplicate. Please edit, or include last initial or name.`);
        person.warning = true;
        nameCheck = false;
      }
  ...
  return (
    <div className="generator-wrap">
      ...
      <p className={showWarning ? 'warning-message show-warning' : 'warning-message'}>{warningText}</p>
      <Participants 
